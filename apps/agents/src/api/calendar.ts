...
export async function getMicrosoftAuthUrl(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to generate Microsoft auth URL'
    });
  }
}
...
export async function handleMicrosoftCallback(req: Request, res: Response) {
  try {
    ...
    const accessToken = await CalendarIntegrationService.handleMicrosoftCallback(code, state);
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to connect Microsoft Calendar'
    });
  }
}
 ...
 * POST /api/calendar/microsoft/import
 */
export async function importMicrosoftCalendar(req: Request, res: Response) {
  try {
    const userId = req.authUserId;
    const { accessToken } = req.body;
    ...
    const events = await CalendarIntegrationService.importMicrosoftEvents(userId, accessToken);
    ...
    res.json({
      ...
      dateRange,
      analysisJobId,
      message: `Imported ${eventCount} events from Microsoft Calendar`
    });
  } catch (error: any) {
    console.error('Error importing Microsoft Calendar:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to import Microsoft Calendar'
    });
  }
}
