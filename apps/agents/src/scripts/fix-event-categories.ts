 ...
 *
 * Usage:
 *   npx tsx src/scripts/fix-event-categories.ts [--dry-run]
 */
...
// Script is in apps/agents/src/scripts, so go up 4 levels to project root
...
async function main() {
  ...
  if (remainingEvents && remainingEvents.length > 0) {
    ...
    console.log('   No remaining events without category');
  }
