...
export const createRatingTool = tool(
  async ({ topic, score, description, aspectId, notes }, config) => {
    ...
    try {
      ...
      return `Rating created: "${topic}" = ${score}/5 (ID: ${rating.id})`;
    } catch (error) {
  ...
  {
    name: "create_rating",
    description: "Create or update a rating score (1-5) for a topic. Use this for tracking metrics like sleep quality, energy, mood, work-life balance, etc.",
    schema: z.object({
      topic: z.string().describe("The topic being rated (e.g., 'Sleep quality', 'Energy level', 'Work satisfaction')"),
      score: z.number().min(1).max(5).describe("Rating score from 1 (lowest) to 5 (highest)"),
