...
export const removeAspectMemberTool = tool(
  async ({ aspectId, aspectName, memberUserId }, config) => {
    ...
    const result = await sharedAspectService.removeMember(targetAspectId, userId, member.id);

    if (!result.success) {
      return `Failed to remove member: ${result.error}`;
    }

    return "Member removed from the shared aspect.";
  },
  {
    name: "remove_aspect_member",
    description: "Remove a member from a shared aspect. Only the aspect owner can remove members. Supports aspect lookup by name or direct ID.",
    schema: z.object({
      ...
      memberUserId: z.string().uuid().describe("User UUID of the member to remove"),
    }),
  }
);
