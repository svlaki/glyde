...
export const checkInGoalTool = tool(
  ...
  {
    name: "check_in_goal",
    description: "Record a goal check-in.",
    schema: z.object({
      goalId: z.string().describe("Goal UUID"),
      progressUpdate: z.string().optional().nullable().describe("Progress update"),
      moodRating: z.number().min(1).max(5).optional().nullable().describe("Mood 1-5"),
      confidenceLevel: z.number().min(1).max(5).optional().nullable().describe("Confidence 1-5"),
      obstaclesEncountered: z.array(z.string()).optional().nullable().describe("Obstacles"),
      winsAndProgress: z.array(z.string()).optional().nullable().describe("Wins"),
      nextSteps: z.array(z.string()).optional().nullable().describe("Next steps"),
      reflectionNotes: z.string().optional().nullable().describe("Notes"),
    }),
  }
);
