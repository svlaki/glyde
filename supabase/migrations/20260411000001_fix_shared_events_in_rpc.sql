-- Fix: restore shared event logic in get_events_with_aspects
-- The 20260408 reminder_minutes migration dropped the event_members JOIN,
-- EXISTS subquery, and user_role column, so accepted shared events stopped
-- appearing in the invitee's calendar.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(em.role, 'owner')::TEXT as user_role,
    e.reflection,
